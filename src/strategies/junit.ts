...
export default class JUnitStrategy {
  ...
  private precomputeTestLists(): void {
    ...
    log.info(
      `Found ${timings.length} testcase timings, which is ${(
        (timings.length / this.allTestNames.length) *
        100
    ...
    const medianTime: number =
      timings.length === 0 ? 0.1 : timings[timings.length / 2].timing;
